...
# Find the start index of the mock pilgrimage packages
match = re.search(r'\s*\{\s*id:\s*"pkg-chardham",', content)
if match:
    start_idx = match.start()
    
    end_match = re.search(r'\];\s*export const PACKAGE_CATEGORIES', content)
    if end_match:
